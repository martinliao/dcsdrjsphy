...
class Online_app_model extends MY_Model
{
    ...
    public function getList($conditions=array())
    {
        ...
        if (isset($conditions['where_not_in'])) {
            $params['where_not_in'] = $conditions['where_not_in'];
            unset($conditions['where_not_in']);
        }
        if (isset($conditions['where_in'])) {
            $params['where_in'] = $conditions['where_in'];
            unset($conditions['where_in']);
        }
        if (isset($conditions)) {
    ...
    public function getMaxOrder($conditions=array())
    {
        ...
        if(isset($conditions['beaurau_id'])){
            $this->db->where("beaurau_id", $conditions['beaurau_id']);
        }
    ...
    public function getThisMaxOrder($conditions=array())
    {
        $this->db->select("IFNULL(max(insert_order)+1,1) max_order");
        $this->db->from($this->table);
        $this->db->where("year", $conditions['year']);
        $this->db->where("class_no", $conditions['class_no']);
        $this->db->where("term", $conditions['term']);
        $this->db->where("beaurau_id", $this->flags->user['bureau_id']);
        $this->db->where_not_in("yn_sel", array('6'));
        $query = $this->db->get();
        $data = $query->row_array();
