...
class Export_score_model extends MY_Model
{
    ...
    public function getRealEndTime($year, $term, $class_no)
    {
        $this->db->select('*');
        $this->db->from('periodtime');
        $this->db->where("year",$year);
        ...
        $this->db->where("class_no",$class_no);
        $this->db->where("course_code != 'O00003' and course_date is not null");
        $this->db->order_by('course_date desc,to_time desc');
        $this->db->limit(1);
        $query = $this->db->get();
        $data = $query->result_array();
        return $data;
