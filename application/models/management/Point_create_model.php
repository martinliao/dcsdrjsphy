...
class Point_create_model extends MY_Model
{
    ...
    public function getScoreInfoByPkey($eYear, $eClass_no, $eTerm, $scoredOnlyAndSort=false, $idList=null)
    {
        ...
        $sql .= "b.name, b.email, b.office_email,\n";
        ...
        $i = 1;
        foreach($grade as $grade_row){
            $sql .= "left join online_app_score a{$i} on a.year = a{$i}.year and a.class_no = a{$i}.class_no and a.term = a{$i}.term and a.id = a{$i}.id and a{$i}.grade_type = '{$grade_row['grade_type']}' \n";
            $grades['p' . $i] = 's' . $i;
            $i = $i + 1;
        }
        ...
        if (!empty($idList) && is_array($idList)) {
            for($idListi=0; $idListi<count($idList); $idListi++){
                $idList[$idListi] = $this->db->escape(addslashes($idList[$idListi]));
            }            
