...
class Point_print_model extends MY_Model
{
    ...
    {
    	$params = array(
            'select' => ' online_app.year, online_app.class_no, online_app.term, r.class_name, r.seq_no',
            'order_by' => 'online_app.year,online_app.class_no,online_app.term',
        );

        $params['join'] = array(
                    array(
                        'table' => "require r",
                        'condition'=>'r.year = online_app.year and r.class_no = online_app.class_no and r.term = online_app.term',
                        'join_type'=>'left',
                    ),
        ...
        if (isset($attrs['class_no'])) {
            $like_class_no = array(
                array('field' => 'online_app.class_no', 'value'=>$attrs['class_no'], 'position'=>'both')
            );
    ...
    public function get_count($year, $class_no, $term)
    {
    ...
    public function get_app_name($year, $class_no, $term,$condition = NULL)
    {
        $this->db->select("v.name,o.st_no");
        $this->db->from('online_app o');
        $this->db->join('BS_user v', "o.id=v.idno", 'left');
        $this->db->where("year",$year);
        $this->db->where("term",$term);
        ...
        if(is_null($condition)){
            $this->db->where("yn_sel IN ('1', '3', '4', '5', '8')");
        }else{
            // $this->db->where("yn_sel IN ('1', '3', '5', '8')");
            $this->db->where("yn_sel",1);
            $this->db->order_by("o.st_no");
        }
