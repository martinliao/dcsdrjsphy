...
class Bureau_finish_count_model extends Common_model
{
    public function getBureauFinishCountData($queryYear, $queryStartDate, $queryEndDate, $type)
    {
    ...
    public function getBureaus($queryStartDate, $queryEndDate)
    {
        $sql = "SELECT DISTINCT
                    ...
                    WHERE
                            ...
                            and (bc.abolish_date is null or bc.abolish_date='' or date_format(bc.abolish_date,'yyyymmdd') between " . $this->db->escape(addslashes($queryStartDate)) . " and " . $this->db->escape(addslashes($queryEndDate)) . "  or date_format(bc.abolish_date,'yyyymmdd')>=" . $this->db->escape(addslashes($queryEndDate)) . " )
                        ...
                        LEFT JOIN (
                                ...
                                (
                                        SELECT DISTINCT
                                                r.type, r.beaurau_id
                                        FROM `require` r
                                        WHERE r.class_status IN ('2', '3') AND r.beaurau_id IS NOT NULL    AND $time_sql  AND r.year=".$this->db->escape(addslashes($queryYear))."
                                ) zz
                                group by type
                        ) X
                        ON r.type=X.type
    ...
    public function csvexport($filename, $query_start_date, $query_end_date, $data, $dayOfWeek)
    {
        ...
        echo iconv('UTF-8', 'BIG5', "各類班期局處結訓人數統計表\r\n");
