...
class TCPDF {
	...
	public function Output($name='doc.pdf', $dest='I') {
		...
		if ($this->sign) {
			...
			// read signature
			//$signature = file_get_contents(K_PATH_CACHE.basename($tempsign));
		...
		switch($dest) {
			case 'I': {
				...
				if (php_sapi_name() != 'cli') {
					...
					header('Content-Disposition: inline; filename="'.$name.'"');  //20210804 改善中文字存檔問題！
			...
			case 'D': {
				...
				header('Content-Disposition: attachment; filename="'.$name.'"');  //mark 改為支持輸出中文檔名
