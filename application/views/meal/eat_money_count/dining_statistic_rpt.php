...
echo '<tr>';
  ...
  for ($i=0; $i < sizeof($rsA); $i++) { 
    $fields=$rsA[$i]; 
    echo '<td nowrap align="center" valign="top" width="80" colspan="2">' . $fields['worker_name'] . '</td>';
  }
  if (sizeof($rsA) < 5){
    $space_cell = 5 - sizeof($rsA); 
    for ($i=0;$i<$space_cell;$i++){
      echo '<td nowrap width="80" colspan="2"></td>';
    }
  }                                                      
echo '</tr>';
...
echo '<tr>';
  ...
  for ($i=0; $i < sizeof($rs); $i++) { 
    ...
    echo '<div style="height:100px;layout-flow:vertical-ideographic;text-align:left">' . $fields['class_name'] . '第' . $fields['term'] . '期</div>';
    echo '</td>';
  }
  if (sizeof($rs) < 15){
    $space_cell = 15 - sizeof($rs); 
  ...
  for ($i=0; $i < sizeof($rsA); $i++) { 
    $fields=$rsA[$i];
    echo '<td align="center" valign="top" rowspan="2" colspan="2">';
...
(
 ...
 from dining_teacher
 where use_date = date(".$this->db->escape(addslashes($query_dt)).") and dining_type = 'A' and type != '2'
...
(
 ...
 from dining_teacher
 where use_date = date(".$this->db->escape(addslashes($query_dt)).") and dining_type = 'B' and type != '2'
 group by year, class_no, term, use_date
...
(
 ...
 from dining_teacher
 where use_date = date(".$this->db->escape(addslashes($query_dt)).") and dining_type = 'C' and type != '2'
 group by year, class_no, term, use_date
...
echo '<tr>';
  echo '<td align="center" rowspan="6">' . htmlspecialchars($d1['dt_nm'], ENT_HTML5|ENT_QUOTES) . '<br>' . htmlspecialchars($weekNM[$d1['week_nm']], ENT_HTML5|ENT_QUOTES) . '</td>';
  echo '<td align="center" height="40" rowspan="2">早餐</td>';
  // $rs2->MoveFirst();  
  for ($i=0; $i < sizeof($rs2); $i++) { 
    $d2=$rs2[$i];
    echo '<td align="center" valign="top">';
    echo htmlspecialchars($d2['m_name'], ENT_HTML5|ENT_QUOTES) . "<br>";
    echo '</td>';
  }
  if (sizeof($rs) < 15){
    $space_cell = 15 - sizeof($rs); 
...
echo '<tr>';
  ...
  for ($i=0; $i < sizeof($rs2); $i++) { 
    $d2=$rs2[$i];
    echo '<td align="center" valign="top">';
    if($d2['m_cnt']!=0){echo htmlspecialchars($d2['m_cnt'], ENT_HTML5|ENT_QUOTES) . "<br>";}
    echo '</td>';
...
echo '<tr>';
  echo '<td align="center" height="40" rowspan="2">午餐</td>';
  // $rs2->MoveFirst();  
  for ($i=0; $i < sizeof($rs2); $i++) { 
    $d2=$rs2[$i];
    echo '<td align="center" valign="top">';
...
echo '<tr>';
  ...
  for ($i=0; $i < sizeof($rs2); $i++) { 
    $d2=$rs2[$i];
    echo '<td align="center" valign="top">';
    if($d2['l_cnt']!=0){echo $d2['l_cnt'] . "<br>";}
    echo '</td>';
    if($d2['l_teach_cnt']!=""){$tot1 = $tot1 + $d2['l_teach_cnt'];}
...
for ($i=0; $i < sizeof($rsA1); $i++) { 
  $fields=$rsA1[$i];
  echo '<td nowrap align="center">早餐';
  ...
  echo '<td nowrap align="center">' . htmlspecialchars($fields['NUM'], ENT_HTML5|ENT_QUOTES) . '</td>';
}
if (sizeof($rsA1) < 5){
...
for ($i=0; $i < sizeof($rsA1); $i++) { 
  $fields=$rsA1[$i];
  echo '<td nowrap align="center">午餐';  
...
for ($i=0; $i < sizeof($rsA1); $i++) { 
  $fields=$rsA1[$i];
  echo '<td nowrap align="center">晚餐';  
...
for ($i=0; $i < sizeof($rsA1); $i++) { 
  $fields=$rsA1[$i];
  echo '<td nowrap align="center">結訓餐';
...
//末列2
//--------------------------------------------------------------------------------------------------
$sql = "select nvl(sum(tot),0) as tot from ( " . 
       "select nvl(amount_1,0) + nvl(amount_2,0) + nvl(amount_3,0) as tot from dining_student " . 
       "where use_date {$where} " .
       ")a";
$totAmt = QueryToArray($this->db->query($sql))[0]['tot'];
...
echo '<tr>';
  ...
  for ($i=0; $i < sizeof($rs); $i++) { 
    $fields=$rs[$i];
    echo '<td align="center" valign="top">' . htmlspecialchars($fields['ROOM_CODE'], ENT_HTML5|ENT_QUOTES) . '</td>';
  }
  if (sizeof($rs) < 15){
    $space_cell = 15 - sizeof($rs); 
    for ($i=0;$i<$space_cell;$i++){
  ...
  echo '<td align="center" width="40">';
  echo htmlspecialchars($totAmt, ENT_HTML5|ENT_QUOTES);
  echo '</td>';
...
left join venue_time b on a.room_id = b.room_id and a.use_period = b.price_t 
where a.appi_id is not null and a.use_period in ('11','12','13','14')  and a.use_date {$where}) as zz group by appi_id ) B 
ON A.APPI_ID = B.APPI_ID 
where A.appi_id in ( select distinct appi_id from room_use where use_period in ('11','12','13','14') 
and appi_id is not null and use_date {$where}) ORDER BY A.APPI_ID
...
for ($i=0; $i < sizeof($rsA1); $i++) { 
  $fields=$rsA1[$i];
  echo '<td nowrap align="center">合計</td>';  
  echo '<td nowrap align="center">' . htmlspecialchars($fields['AMT'], ENT_HTML5|ENT_QUOTES) . '</td>';
}
if (sizeof($rsA1) < 5){
  $space_cell = 5 - sizeof($rsA1); 
  for ($i=0;$i<$space_cell;$i++){
