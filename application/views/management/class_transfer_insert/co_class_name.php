...
function page(n){
  <?php
  echo "var cur_page = " . $filter['class_page'] . ";";
  ...
  ?>
	...
	obj.class_page.value = tmp;
  obj.submit();
}
function popupOK(x){

  window.opener.document.getElementById("<?=$savefeild;?>").value = x.value;
  <?php if($savefeild == 'addClass_before'){ ?>
    window.opener.selClassOK();
  <?php }else{ ?>
    window.opener.selClassOK2();
  <?php } ?>
  window.close();
}
</script>
...
<div id="list1">
請輸入關鍵字
<input type="text" name="class_q" id="class_q" value="<?=$filter['class_q'];?>">
<input type="button" value="查詢" class="button" style="FONT-SIZE:12px;" onclick="doQuery()">
<?php
echo '<input type="hidden" name="class_page" id="class_page" value="' . $filter['class_page'] . '">';
echo '<input type="hidden" name="field" id="field" value="' . $savefeild . '">';
?>
  ...
  <tr>
    <td bgcolor="#909397">
      ...
      <table width="100%" border="0" cellspacing="1" cellpadding="1">
      		  ...
      		  <?php
      		  echo "頁次:" . $filter['class_page'] . "/" . $total_page;
            ?>
            </span>
      		  <?php
            $p1 = (($filter['class_page']==1) || ($filter['class_page']==0)) ? "disabled" : 'onclick="page(-1)"';
            $p2 = (($filter['class_page']==1) || ($filter['class_page']==0)) ? "disabled" : 'onclick="page(-2)"';
            $p3 = (($filter['class_page']==$total_page) || ($filter['class_page']==0)) ? "disabled" : 'onclick="page(-3)"';
            $p4 = (($filter['class_page']==$total_page) || ($filter['class_page']==0)) ? "disabled" : 'onclick="page(-4)"';
        ...
        <?php
        foreach($class_list as $fields){
          echo "<tr>";
          echo '<td align="center" bgcolor="#ffffff">';
          $value = $fields['class_no'] . "::" . $fields['class_name'];
