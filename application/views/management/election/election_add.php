		...
		<div class="panel panel-default">
			...
			<div class="panel-body">
				<form id="actQuery" method="POST" >
					...
					<br>
					<a name='btn_a1' class="btn btn-default" title='依可報名人數選員' onclick="newSort('<?= $class['limil_max'];?>')" <?=($class['disableCount']>0)?'disabled':'';?> > 依可報名人數選員 </a>
					<a name='btn_a2' class="btn btn-default" title='依設定人數選員' onclick="showText()" <?=($class['disableCount']>0)?'disabled':'';?> > 依設定人數選員 </a>
					<a name='btn_a3' class="btn btn-default" title='未錄取名單' onclick="no_detail()" <?=($class['disableCount']>0)?'disabled':'';?> > 未錄取名單 </a>
					<table width="100%" class="table table-bordered table-striped table-condensed" name="tbdata" id="tbdata">
						...
						<tbody>
							...
							<?php foreach($regist_list as $row){ ?>
									...
									if($row['yn_sel'] == 1 || $row['yn_sel'] == 4 || $row['yn_sel'] == 5 ){
										//如果狀態為結訓  || $row['yn_sel'] == 8
										$isChecked = "checked";
										$isEnabled = "disabled";
									}
									if($row['yn_sel'] == 4){
					...
					<div>
						<input type='hidden' id='scount' name='scount' value='<?=count($regist_list);?>' >
...
$(function(){
  ...
  //alert(id1+" "+num);
//if(num != '' || id1 !='') {
	for(var k=0;k<$("input[name=chkDateEdit]").length;k++) {

	// alert(k);
		if($("input[name=chkDateEdit]").eq(k).attr("checked") != 'checked') {
			 $("#tbdata tr").eq(k+1).css("background-color", "yellow");
			 // $("input[name=txtSTNO]").eq(k).val("");
		}else {
			 $("#tbdata tr").eq(k+1).css("background-color", "#ffffff");
		}
