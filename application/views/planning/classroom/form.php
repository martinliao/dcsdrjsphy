    ...
    <div class="tab-pane col-xs-12">
        ...
        <table class="table table-hover" style="margin-bottom: 10px;">
            ...
            <tbody id="bookinged">
                ...
                <tr>
                    <td>
                        <button type="button" class="btn btn-outline btn-danger btn-xs btn-toggle" title="刪除" onclick="del_booking('<?=$row['room_id'];?>', '<?=$row['cat_id'];?>', '<?=$row['booking_period'];?>', '<?=$row['start_date'];?>', '<?=$row['end_date'];?>', '<?=$row['seq_no'];?>');">
                            刪除
                        </button>
                    </td>
...
var del_booking = function(room_id, cat_id, booking_period, start_date, end_date, seq_no2) {
        ...
        $.ajax({
            ...
            success: function(response){
                        if (response.status) {
                            //location.reload();
                            document.location.href="<?=base_url('planning/classroom/add/');?>"+ seq_no2; //20210806 Roger 刪涂完之後直接轉址，才不會亂跑
                        } else {
...
var show_booking = function(booking_data) { //20210806 Roger 傳給del_booking 的值重寫
    $("#bookinged").empty();
    var html = '';
    var rroom_id ='';
    for (k in booking_data) {
        var row = booking_data[k];
        rroom_id = "'"+row.room_id+"', '"+row.cat_id+"', '"+row.booking_period+"', '"+row.start_date+"', '"+row.end_date+"', '"+row.seq_no+"'";
        html += '<tr>';
        html += '   <td>';
        html += '   <button type="button" class="btn btn-outline btn-danger btn-xs btn-toggle" title="刪除" onclick="del_booking('+ rroom_id +');">刪除</button>';
        html += '   </td>';
