...
class Election extends MY_Controller
{
    public function __construct()
    {
        ...
        if (!isset($this->data['filter']['allClass'])) {
            $this->data['filter']['allClass'] = '';
        }
    }
    ...
    public function index()
    {
        ...
        foreach ($this->data['list'] as & $row) {
            $row['link_add'] = base_url("management/election/election_add/{$row['seq_no']}?{$_SERVER['QUERY_STRING']}");
        }
        $this->load->library('pagination');
        $config['base_url'] = base_url("management/election?". $this->getQueryString(array(), array('page')));
        $config['total_rows'] = $total;
        $config['per_page'] = $rows;
        $this->pagination->initialize($config);

        $this->data['link_refresh'] = base_url("management/election/");

        $this->layout->view('management/election/list', $this->data);
    }
    ...
    public function election_add($seq_no=NULL)
    {
        ...
        $this->data['link_ids'] = base_url("management/election/election_add/{$seq_no}");
        
    	$this->data['link_cancel'] = base_url("management/election/?{$_SERVER['QUERY_STRING']}");
    	// jd($this->data['class'],1);
    	$this->layout->view('management/election/election_add', $this->data);
    }
            ...
            switch ($action) {
                case 'do_election':
                    ...
                    if($error === TRUE){
                        ...
                        $this->Require_model->updateSelectNumber($conditions, $post['scount']);
