...
class Teacher_evaluate_rate extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $this->data['link_refresh'] = base_url("old_report/teacher_evaluate_rate/");
        $this->data['link_old'] = base_url("old_report/old_teacher_evaluate_score/");
        $this->data['datas'] = array();
        if($this->input->get()){
            $list = $this->old_report_model->getTeacherRateList($teacher_name, $teacher_id, $course_name, $class_name, $start_date, $end_date);
            ...
            for($i=0;$i<count($list);$i++){
                $cnt = $this->old_report_model->get_teacher_cnt_gender_rate($list[$i]['class_id'],$list[$i]['year'],$list[$i]['term'],$list[$i]['course_code']);
                $list[$i]['score_list'] = $this->old_report_model->get_teacher_rate($list[$i]['class_id'],$list[$i]['year'],$list[$i]['term'],$list[$i]['course_code'],$list[$i]['id']);
        ...
        $this->layout->view('old_report/teacher_evaluate_rate/list',$this->data);
    }
