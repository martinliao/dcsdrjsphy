...
class Teacher_manger extends MY_Controller
{
    public function __construct()
    {
        ...
        $this->data['choices']['education'] = $this->teacher_model->getEducation();
        ...
        $this->data['choices']['identity_type'] = $this->teacher_model->identity_type;
    ...
    public function index()
    {
        ...
        if (empty($this->input->get())){
            ...
            $this->data['filter']['total'] = $total = $this->teacher_model->getListCount($attrs);
            ...
            $this->data['list'] = $this->teacher_model->getList($attrs);
        }
        
        // jd($this->data['list']);

        foreach ($this->data['list'] as & $row) {
            $row['link_edit'] = base_url("data/teacher_manger/edit/{$row['id']}/?{$_SERVER['QUERY_STRING']}");
        }
        $this->load->library('pagination');
        $config['base_url'] = base_url("data/teacher_manger?". $this->getQueryString(array(), array('page')));
        $config['total_rows'] = $total;
        $config['per_page'] = $rows;
        $this->pagination->initialize($config);

        $this->data['link_add_teacher'] = base_url("data/teacher_manger/add/?{$_SERVER['QUERY_STRING']}");
        $this->data['link_delete'] = base_url("data/teacher_manger/delete/?{$_SERVER['QUERY_STRING']}");
        $this->data['link_refresh'] = base_url("data/teacher_manger/");

        $this->layout->view('data/teacher_manger/list', $this->data);
    }
    ...
    public function add()
    {
        ...
        $this->data['form'] = $this->teacher_model->getFormDefault();
        ...
        */
        ...
        if (isset($id)){
            if (isset($teacher_type)){
                //新增 講師/助教 身份
                $this->data['form'] = $this->teacher_model->getFormDefault($this->teacher_model->_get($id,'idno'));
                $conditions = array(
        ...
        if($this->input->get('key')){
            $add_id = $this->teacher_model->getTeacherIdByIdno(strtoupper(trim($this->input->get('key'))));
            
            if($add_id > 0){
                redirect(base_url("data/teacher_manger/edit/".$add_id.'/?'));
            } else {
        ...
        if ($post = $this->input->post()) {
            ...
            if ($this->_isVerify('add',null,$identity_type) == TRUE) {
                ...
                $city_data = $this->teacher_model->get_city($post['zipcode']);
                $post['county'] = $city_data['city'];
                $post['district'] = $city_data['subcity'];
                $saved_id = $this->teacher_model->_update_or_create($conditions, $post);
                if ($saved_id) {
        ...
        $this->data['link_save2'] = base_url("data/teacher_manger/add/?{$_SERVER['QUERY_STRING']}");
        $this->data['link_cancel'] = base_url("data/teacher_manger/?{$_SERVER['QUERY_STRING']}");
        $this->data['show_bank'] = base_url("data/teacher_manger/show_bank");
        $this->layout->view('data/teacher_manger/add', $this->data);
    }
    ...
    public function edit($id=NULL)
    {
        ...
        if ($post = $this->input->post()) {
            ...
            if($this->form_validation->run()==FALSE){
                $this->setAlert(2,'帳號請填入數字，不可有特殊字元');
                redirect(base_url("data/teacher_manger/edit/{$id}?{$_SERVER['QUERY_STRING']}"));
            }*/

            

            $old_data = $this->teacher_model->_get($id);
            ...
            if ($this->_isVerify('edit', $old_data, $identity_type) == TRUE) {
                ...
                $post['district'] = $city_data['subcity'];
                $post['del_flag'] = 'N';
                $rs = $this->teacher_model->_update($id, $post);
                unset($post['del_flag']);
                
                $this->teacher_model->updateAll($post["idno"],$post);//同時更改講座及助教的資料
                //}
        ...
        $this->data['link_save2'] = base_url("data/teacher_manger/edit/{$id}/?{$_SERVER['QUERY_STRING']}");
        $this->data['link_cancel'] = base_url("data/teacher_manger/?{$_SERVER['QUERY_STRING']}");
        $this->data['show_bank'] = base_url("data/teacher_manger/show_bank");
        $this->layout->view('data/teacher_manger/edit', $this->data);
    }
    ...
    public function delete()
    {
        if ($post = $this->input->post()) {
            ...
            //     jd($post['rowid'],1);
            // }
            // $this->teacher_model->canteach_delete($post);
            foreach ($post['rowid'] as $id) {
                $fields = array(
                    'del_flag' => 'Y',
                );
                $rs = $this->teacher_model->update($id, $fields);
            }
    ...
    {
        $config = $this->teacher_model->getVerifyConfig();
        if($identity_type == 4){
        ...
        /* 
        系統目前設計以郵寄區號(3碼)來分辨地址
        但是因嘉義市東區西區郵遞區號前三碼都是 600 為了分辨 東區與西區 把西區郵遞區號改成大型 ６００
        此改動導致前後端判斷時有誤 有另外判斷 ６００ 的都是為了解決判斷錯誤的問題
        */
        if ($this->input->post('zipcode') === '６００'){
            unset($config['zipcode']);
        }
    ...
    public function teacher_combo_import()
    {
            ...
            if (isset($_FILES['aCSV']) && $_FILES['aCSV']['tmp_name'] != '') {
                ...
                while(! feof($file))
                {
                    ...
                    if($EACH_STATUS == true){
                        ...
                        $city_data = $this->teacher_model->get_city($fields['zipcode']);
                        ...
                        $ifExist = $this->teacher_model->get($conditions);
                        $fields['del_flag'] = 'N';

                        if($ifExist){
                            $rs = $this->teacher_model->update($conditions, $fields);
                            ...
                            $saved_id = $this->teacher_model->insert($fields);
                            $massage .= "第{$i}筆 新增成功 <br>\n";
                            $this->teacher_log_model->insert($log_fields);
                        }
                    }else{
        ...
        $this->data['import_save'] = base_url("data/teacher_manger/teacher_combo_import");
        $this->data['import_cancel'] = base_url("data/teacher_manger/?{$_SERVER['QUERY_STRING']}");
        $this->layout->view('data/teacher_manger/teacher_combo_import', $this->data);
    }
    ...
    public function show_bank()
    {
        ...
        $this->load->view('data/teacher_manger/pop_bank', $this->data);
    }
    ...
    public function edit_log(){
        ...
        $teacher_type = $this->getFilterData('teacher_type');
        $this->data['teacher'] = $this->teacher_model->_get($idno, 'idno');
        ...
        $this->layout->view('data/teacher_manger/edit_log', $this->data);
    }
    ...
    public function teacher_is_unique(){
        ...
        if (empty($idno) || empty($teacher_type)){
            ...
            $result = empty($this->teacher_model->getTeacherByType($idno, $teacher_type));
            ...
            switch ($action) {
                ...
                case 'getAutoID':
                    ...
                    if($error === TRUE){
                    	...
                    	$autoID = $this->teacher_model->getAutoID($post['type']);
                    	$result['status'] = TRUE;
