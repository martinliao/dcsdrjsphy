...
class Bank_code extends MY_Controller
{
	...
	public function index()
	{
		...
		foreach ($this->data['list'] as & $row) {
			$row['link_edit'] = base_url("data/bank_code/edit/{$row['id']}/?{$_SERVER['QUERY_STRING']}");
		}
		$this->load->library('pagination');
        $config['base_url'] = base_url("data/bank_code?". $this->getQueryString(array(), array('page')));
        $config['total_rows'] = $total;
        $config['per_page'] = $rows;
        $this->pagination->initialize($config);

		$this->data['link_add'] = base_url("data/bank_code/add/?{$_SERVER['QUERY_STRING']}");
		$this->data['link_delete'] = base_url("data/bank_code/delete/?{$_SERVER['QUERY_STRING']}");
		$this->data['link_refresh'] = base_url("data/bank_code/");

		$this->layout->view('data/bank_code/list', $this->data);
	}

	public function add()
	{
		$this->data['page_name'] = 'add';
		$this->data['form'] = $this->bank_code_model->getFormDefault();
		...
		if ($post = $this->input->post()) {
			if ($this->_isVerify('add') == TRUE) {
				...
				$post['cancel_flag'] = '';
				$saved_id = $this->bank_code_model->_insert($post);
				if ($saved_id) {
					$fields = array(
						'type_id' => '14',
				...
				redirect(base_url('data/bank_code'));
			}
		}

		$this->data['link_save'] = base_url("data/bank_code/add/?{$_SERVER['QUERY_STRING']}");
		$this->data['link_cancel'] = base_url("data/bank_code/?{$_SERVER['QUERY_STRING']}");
		$this->layout->view('data/bank_code/add', $this->data);
	}

	public function edit($id=NULL)
	{
		$this->data['page_name'] = 'edit';
		$this->data['form'] = $this->bank_code_model->getFormDefault($this->bank_code_model->get($id));
		...
		if ($post = $this->input->post()) {
			...
			if ($this->_isVerify('edit', $old_data) == TRUE) {
				$post['modify_time'] = date('Y-m-d H:i:s');
				$post['modify_user'] = $this->flags->user['id'];
				$rs = $this->bank_code_model->_update($id, $post);
				if ($rs) {
					$fields = array(
						'type_id' => '14',
						'item_id' => $post['item_id'],
				...
				redirect(base_url("data/bank_code/?{$_SERVER['QUERY_STRING']}"));
			}
		}

		$this->data['link_save'] = base_url("data/bank_code/edit/{$id}/?{$_SERVER['QUERY_STRING']}");
		$this->data['link_cancel'] = base_url("data/bank_code/?{$_SERVER['QUERY_STRING']}");
		$this->layout->view('data/bank_code/edit', $this->data);
	}
	...
	public function delete()
	{
		if ($post = $this->input->post()) {
			foreach ($post['rowid'] as $id) {
				...
				$this->code_history_model->insert($fields);
				$rs = $this->bank_code_model->delete($id);
			}
