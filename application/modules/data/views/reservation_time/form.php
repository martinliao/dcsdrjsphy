...
<form id="data-form" role="form" method="post" action="<?=$link_save;?>">
    ...
    <div class="form-group col-xs-6 required <?=form_error('remark')?'has-error':'';?>">
        <label class="control-label">簡稱</label>
        <input class="form-control" name="remark" placeholder="" value="<?=set_value('remark', $form['remark']); ?>">
        <?=form_error('remark'); ?>
    </div>
    <div class="form-group col-xs-6 required <?=form_error('start_time')?'has-error':'';?>">
        <label class="control-label">起始時間</label>
        <input class="form-control" name="start_time" placeholder="" value="<?=set_value('start_time', $form['start_time']); ?>">
        <?=form_error('start_time'); ?>
    </div>
    <div class="form-group col-xs-6 required <?=form_error('end_time')?'has-error':'';?>">
        <label class="control-label">結束時間</label>
        <input class="form-control" name="end_time" placeholder="" value="<?=set_value('end_time', $form['end_time']); ?>">
        <?=form_error('end_time'); ?>
    </div>
