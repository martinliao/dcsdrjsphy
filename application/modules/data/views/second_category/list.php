                    ...
                    <form id="filter-form" role="form" class="form-inline">
                        ...
                        <div class="col-xs-12" >
                            <div class="form-group">
                                ...
                                <?php
                                    echo form_dropdown('type', $choices['type'], $filter['type'], 'class="form-control"');
                                ?>
                            </div>
                            <div class="form-group">
                                <label class="control-label">次類別代碼</label>
                                <input type="text" class="form-control" name="item_id" value="<?=$filter['item_id'];?>">
                            </div>
                            <div class="form-group">
                    ...
                    <table class="table table-bordered table-condensed table-hover">
                        <thead>
                            <tr>
                                ...
                                <th class="sorting<?=($filter['sort']=='series_name asc')?'_asc':'';?><?=($filter['sort']=='series_name desc')?'_desc':'';?>" data-field="series_name" >主類別</th>
                                <th class="sorting<?=($filter['sort']=='item_id asc')?'_asc':'';?><?=($filter['sort']=='item_id desc')?'_desc':'';?>" data-field="item_id" >次類別代碼</th>
                                <th class="sorting<?=($filter['sort']=='name asc')?'_asc':'';?><?=($filter['sort']=='name desc')?'_desc':'';?>" data-field="name" >次類別名稱</th>
                                <th class="sorting<?=($filter['sort']=='short_name asc')?'_asc':'';?><?=($filter['sort']=='short_name desc')?'_desc':'';?>" data-field="short_name" >次類別簡稱</th>
                                <th class="sorting<?=($filter['sort']=='remark asc')?'_asc':'';?><?=($filter['sort']=='remark desc')?'_desc':'';?>" data-field="remark" >備註</th>
                                <th>功能</th>
                            </tr>
                        </thead>
                        <tbody>
...
$(document).ready(function() {
    ...
    $('#list-form').highlight('<?=$filter['name'];?>');
    <?php } ?>
    <?php if (isset($filter['item_id']) && $filter['item_id'] != ''){ ?>
    $('#list-form').highlight('<?=$filter['item_id'];?>');
    <?php } ?>
});
</script>
