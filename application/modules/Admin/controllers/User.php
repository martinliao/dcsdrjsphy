...
class User extends BackendController
{
	...
	public function index()
	{
			...
			$this->data['users'] = $this->smarty_acl->users(true); // true is object
			foreach ($this->data['users'] as $k => $user) {
				//$this->data['users'][$k]->groups = $this->ion_auth->get_users_groups($user->id)->result();
				//$this->data['users'][$k] = (object) $this->data['users'][$k];
				$this->data['users'][$k]->groups = array('id' => 2, 'name' => 'General User');
			}
			/* Load Template */
			//$this->template->admin_render('admin/users/index', $this->data);
			$this->render_page('users/index', $this->data);
	}
