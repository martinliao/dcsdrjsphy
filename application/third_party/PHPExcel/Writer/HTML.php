...
class PHPExcel_Writer_HTML extends PHPExcel_Writer_Abstract implements PHPExcel_Writer_IWriter
{
    ...
    public function generateHTMLHeader($pIncludeStyles = false)
    {
        ...
        if ($properties->getTitle() > '') {
            $html .= '      <title>' . htmlspecialchars($properties->getTitle(), ENT_HTML5|ENT_QUOTES) . '</title>' . PHP_EOL;
        }
        if ($properties->getCreator() > '') {
            $html .= '      <meta name="author" content="' . htmlspecialchars($properties->getCreator(), ENT_HTML5|ENT_QUOTES) . '" />' . PHP_EOL;
        }
        if ($properties->getTitle() > '') {
            $html .= '      <meta name="title" content="' . htmlspecialchars($properties->getTitle(), ENT_HTML5|ENT_QUOTES) . '" />' . PHP_EOL;
        }
        if ($properties->getDescription() > '') {
            $html .= '      <meta name="description" content="' . htmlspecialchars($properties->getDescription(), ENT_HTML5|ENT_QUOTES) . '" />' . PHP_EOL;
        }
        if ($properties->getSubject() > '') {
            $html .= '      <meta name="subject" content="' . htmlspecialchars($properties->getSubject(), ENT_HTML5|ENT_QUOTES) . '" />' . PHP_EOL;
        }
        if ($properties->getKeywords() > '') {
            $html .= '      <meta name="keywords" content="' . htmlspecialchars($properties->getKeywords(), ENT_HTML5|ENT_QUOTES) . '" />' . PHP_EOL;
        }
        if ($properties->getCategory() > '') {
            $html .= '      <meta name="category" content="' . htmlspecialchars($properties->getCategory(), ENT_HTML5|ENT_QUOTES) . '" />' . PHP_EOL;
        }
        if ($properties->getCompany() > '') {
            $html .= '      <meta name="company" content="' . htmlspecialchars($properties->getCompany(), ENT_HTML5|ENT_QUOTES) . '" />' . PHP_EOL;
        }
        if ($properties->getManager() > '') {
            $html .= '      <meta name="manager" content="' . htmlspecialchars($properties->getManager(), ENT_HTML5|ENT_QUOTES) . '" />' . PHP_EOL;
        }
        ...
        foreach ($pSheet->getDrawingCollection() as $drawing) {
            if ($drawing instanceof PHPExcel_Worksheet_Drawing) {
                if ($drawing->getCoordinates() == $coordinates) {
                    ...
                    $filename = htmlspecialchars($filename, ENT_HTML5|ENT_QUOTES);
...
//        $css = $this->mapBorderStyle($pStyle->getBorderStyle()) . ' #' . $pStyle->getColor()->getRGB();
        ...
        if (is_array($pValues)) {
            ...
            foreach ($pValues as $cellAddress) {
                ...
                if ($cell instanceof PHPExcel_Cell) {
                    ...
                    if ($cell->getValue() instanceof PHPExcel_RichText) {
                        ...
                        foreach ($elements as $element) {
                            ...
                            $cellData .= htmlspecialchars($cellText, ENT_HTML5|ENT_QUOTES);
                        ...
                        $cellData = htmlspecialchars($cellData, ENT_HTML5|ENT_QUOTES);
                ...
                if ($pSheet->hyperlinkExists($coordinate) && !$pSheet->getHyperlink($coordinate)->isInternal()) {
                    $cellData = '<a href="' . htmlspecialchars($pSheet->getHyperlink($coordinate)->getUrl(), ENT_HTML5|ENT_QUOTES) . '" title="' . htmlspecialchars($pSheet->getHyperlink($coordinate)->getTooltip(), ENT_HTML5|ENT_QUOTES) . '">' . $cellData . '</a>';
                }
    ...
    public function formatColor($pValue, $pFormat)
    {
        ...
        // convert to PCDATA
        $value = htmlspecialchars($pValue, ENT_HTML5|ENT_QUOTES);
